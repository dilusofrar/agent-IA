...
STANDARD_START = time(7, 45)
LUNCH_START = time(12, 0)
LUNCH_END = time(13, 0)
STANDARD_END = time(17, 0)
EXPECTED_MINUTES = 8 * 60 + 15
...
def parse_timecard_text(text: str) -> TimeCardAnalysis:
    ...
    return TimeCardAnalysis(
        ...
        days=[
            calculate_day_metrics(current_date, day_map.get(current_date))
...
def calculate_day_metrics(work_date: date, raw_day: RawPunchDay | None) -> DayMetrics:
    holiday_name = get_brazil_holiday_name(work_date)
    weekend = work_date.weekday() >= 5

    if raw_day is None:
        if weekend or holiday_name:
    ...
    if start_dt.time() >= LUNCH_START and start_dt.time() < LUNCH_END:
        issues.append("Entrada registrada durante o intervalo de almoco.")
    if end_dt.time() > LUNCH_START and end_dt.time() <= LUNCH_END:
